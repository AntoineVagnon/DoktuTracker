...
async function checkDates() {
  ...
  const last7Days = allAppointments.filter(apt => 
    apt.status !== 'cancelled' && new Date(apt.date) >= sevenDaysAgo
  ).length;
  
  const last30Days = allAppointments.filter(apt => 
    apt.status !== 'cancelled' && new Date(apt.date) >= thirtyDaysAgo
  ).length;
  
  const last90Days = allAppointments.filter(apt => 
    apt.status !== 'cancelled' && new Date(apt.date) >= ninetyDaysAgo
  ).length;
  
  console.log('\nAppointments by period (excluding cancelled):');
