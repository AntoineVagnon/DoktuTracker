/**
 * Script to create future availability slots for test doctors
 * Run with: npx tsx scripts/create-future-slots.ts
 */
  ...
  for (const slot of slots) {
    try {
      ...
      if (response.ok) {
        ...
        failed++;
        if (failed === 1) {
          console.log(`  ❌ Failed to create slot: ${response.status} ${await response.text()}`);
        }
      }
    } catch (error) {
      failed++;
...
async function main() {
  try {
    console.log('🚀 Creating future availability slots for test doctors\n');

    // Authenticate
    const sessionCookie = await authenticateAsAdmin();

    // Generate slots for next 2 weeks (Oct 15-30, 2025)
    const startDate = new Date('2025-10-15');
    const slots = generateFutureSlots(startDate, 15);

    console.log(`📊 Generated ${slots.length} time slots (weekdays only, Oct 15-30)`);
    console.log(`   Morning slots: 09:00-11:30`);
    console.log(`   Afternoon slots: 14:00-16:30`);

    // Create slots for Dr. Sarah Johnson (ID 8)
    await createSlotsForDoctor(8, slots, sessionCookie);

    // Also create for Dr. Sophie Chen (ID 10)
    await createSlotsForDoctor(10, slots, sessionCookie);

    console.log('\n✅ All done! Slots created successfully.');
    console.log('\nVerify by visiting:');
    console.log(`  https://doktu-tracker.vercel.app/doctor/8`);
    console.log(`  https://doktu-tracker.vercel.app/doctor/10`);
