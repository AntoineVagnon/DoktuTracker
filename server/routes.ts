...
import * as mailgunWebhooks from "./services/mailgunWebhooks";
...
export async function registerRoutes(app: Express): Promise<void> {
  ...
  // Mailgun Webhook Endpoint (public - no auth, signature verified by Mailgun)
  app.post("/api/webhooks/mailgun", async (req, res) => {
    try {
      console.log('📬 Mailgun webhook received:', req.body?.['event-data']?.event);

      await mailgunWebhooks.processMailgunEvent(req.body);

      res.status(200).json({ success: true, message: 'Webhook processed' });
    } catch (error: any) {
      console.error('❌ Mailgun webhook error:', error.message);
      // Still return 200 to prevent Mailgun retries for invalid signatures
      res.status(200).json({ success: false, error: error.message });
    }
  });

  // Email Analytics Endpoints (Admin only)
  app.get("/api/admin/analytics/notification/:id", isAuthenticated, async (req, res) => {
    try {
      const user = req.user as any;
      if (!user || user.role !== 'admin') {
        return res.status(401).json({ message: "Unauthorized" });
      ...
      const { id } = req.params;
      const analytics = await mailgunWebhooks.getNotificationAnalytics(id);
      const events = await mailgunWebhooks.getNotificationEvents(id);
  ...
  app.get("/api/admin/analytics/template/:templateKey", isAuthenticated, async (req, res) => {
    try {
      const user = req.user as any;
      if (!user || user.role !== 'admin') {
        return res.status(401).json({ message: "Unauthorized" });
      ...
      const performance = await mailgunWebhooks.getTemplatePerformance(templateKey);
  ...
  app.get("/api/admin/analytics/templates", isAuthenticated, async (req, res) => {
    try {
      const user = req.user as any;
      if (!user || user.role !== 'admin') {
        return res.status(401).json({ message: "Unauthorized" });
      ...
      const performance = await mailgunWebhooks.getTemplatePerformance();

      res.json({ templates: performance });
    } catch (error) {
      console.error("Error fetching all templates performance:", error);
      res.status(500).json({ message: "Failed to fetch templates performance" });
    }
  });
  ...
  app.get("/api/admin/analytics/dashboard", isAuthenticated, async (req, res) => {
    try {
      ...
      const dashboardData = await mailgunWebhooks.getAnalyticsDashboard(startDate, endDate);
  ...
  app.post("/api/admin/analytics/refresh", isAuthenticated, async (req, res) => {
    try {
      ...
      await mailgunWebhooks.refreshTemplatePerformance();

      res.json({ success: true, message: "Template performance stats refreshed" });
    } catch (error) {
  ...
  // Register doctor documents routes (German medical credential documents)
